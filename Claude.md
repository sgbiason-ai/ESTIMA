...
| **Compteur `P.${counter++}` local dans un composant** (AnalysisTable l'avait depuis l'origine) → numéros ≠ DQE dès le 1er prix répété ; les exports PDF/Excel recevaient déjà le bon `bpuRefMap`, seul l'écran divergeait | passer `refMap` (celui de `useProjectCalculations`) en prop ; toute numérotation affichée OU rapprochée vient de `buildRefMap` — jamais un compteur local (`useBpuData` a déjà le registre, ne pas y toucher) |
| **Reconstruire la numérotation `P.n` au lieu de réutiliser `buildRefMap`**. `bpuNum` est presque toujours **vide** en base : `P.43` n'est écrit nulle part, il est **calculé** à l'export par le registre de prix uniques (`buildRefMap`/`useProjectCalculations`), qui n'incrémente que sur un prix **nouveau**. Une numérotation « un P.n par article » diverge dès le 1er prix répété : 79 lignes → 73 numéros, **65/79 articles mal numérotés**, `P.43` pointant sur un tout autre ouvrage → ligne à désignation retouchée rattachée au mauvais article, le bon reste à **0 €** (−3 405,60 € en prod, remonté comme « écart AE »). Piège de test : fabriquer les `bpuNum` depuis la colonne N° du fichier Excel **masque le bug** (le repli par ref n'est jamais sollicité) | `offerItemMatcher.js` appelle **`buildRefMap(project.chapters, bpuConfig)`** — la fonction même qui numérote le DQE exporté, donc divergence impossible. Passer `bpuConfig` (modes auto/manual/hierarchical). `bpuNum` saisis enregistrés **en premier** (priorité), puis `P.n` **et** `P.0n` (l'export écrit `P.1`, d'anciens fichiers `P.01` ; `normalizeRef` retire le point). Tester avec un projet **sans `bpuNum` ni `uid`** — le cas réel |
...
| **DQE à tranches : quantité importée = 1re ligne, comparée à la somme des tranches** : un bordereau multi-tranches répète chaque article (une section/onglet par tranche) ; « premier non-nul gagne » retenait la qté de la tranche 1 face à la qté MOE **globale** (`clientQtyMaps.global` = somme) → faux `quantity_mismatch` sur TOUT article multi-tranches (52 en prod) → offre auto-classée irrégulière. Danger jumeau : occurrences épuisées + « dernier gagne » enverrait la qté de la tranche 2 sur le mauvais doublon | **sommer** les quantités des lignes d'un même article (import + variante) ; `offerItemMatcher` repart en **début de cycle** quand les occurrences d'une désignation sont épuisées (nouvelle section = nouveau passage sur le bordereau) |
| **PDF d'offre mixte : pages au calque texte corrompu** (police sans table ToUnicode → « ((³ϙ( ») : le fallback OCR global ne se déclenche pas (les pages saines produisent des articles) → les articles portés par les pages corrompues restent à 0 € **en silence** (−49 390,85 € en prod, vus comme « écart AE »). Autre piège : unité composite wrappée (« FT / MOIS » → cellule « MOIS ») rejetée par le catalogue → ligne jamais importée | `isGarbledText` (ratio de glyphes hors latin > 20 %) par page → **OCR ciblé** de ces seules pages (`extractPdfViaOcr(file, onProgress, pageFilter)`), fusion en ordre de pages, warning explicite. Passe cellules : unité **souple** (rejet seulement si > 12 car. ou numérique), la cohérence qté × PU ≈ montant fait foi |
| **Promouvoir un champ texte en riche (RichTextField) sans convertir TOUS ses consommateurs** : un test `(v \|\| '').trim()` compte `<div><br></div>` comme du contenu (champ « requis » jugé rempli alors qu'il est vide) ; un affichage `{v}` sort les balises brutes ; un export `splitTextToSize(v)` imprime le HTML | pour chaque champ promu, convertir les 4 familles : vacuité → `isRichTextEmpty` (`utils/richText.js`), affichage → `RichTextView` ou `htmlToPlainText`, PDF → `richFieldToBlocks`+`drawHtmlBlocks` (mesure exacte via `dryRun`), Excel → `htmlToStyledBlocks`. Recenser par grep `champ \|\| ''\)\.trim` + `{champ}` |
...
| RAO & Analyse des offres | Prod | dépouillement + Rapport + Tableau prix (**sous-chapitres** : entêtes indentées N niveaux + sous-totaux repliables, via `groupPath` ; n° de prix = `refMap` prop), **rapport d'import** (`ImportReportModal` : articles sans prix + lignes non rattachées, `utils/importReport.js`), **texte riche partout** (`RichTextField` + `RichTextView` sur les 8 champs de rédaction ; PDF via `richFieldToBlocks`/`measureHtmlBlocks`/`drawHtmlBlocks` (`dryRun`) ; vacuité via `isRichTextEmpty` — jamais `.trim()` ; bandeau recommandation aplati `htmlToPlainText`), sous-critères, groupements, OAB · `analysis/data`+`rao/data` |

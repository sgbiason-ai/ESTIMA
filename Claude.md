...
| RAO & Analyse des offres | Prod | dépouillement + Rapport + Tableau prix (**sous-chapitres** : entêtes indentées N niveaux + sous-totaux repliables, via `groupPath` ; n° de prix = `refMap` prop), **rapport d'import** (`ImportReportModal` : articles sans prix + lignes non rattachées, `utils/importReport.js`), sous-critères, groupements, OAB · `analysis/data`+`rao/data` |

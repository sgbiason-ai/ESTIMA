...
import { lighten } from './bpuBrandingUtils';
    ...
    <div className="flex-1 overflow-auto bg-slate-200/50 print:p-0 print:overflow-visible">
      ...
      >
        ...
        {pages.map((page, i) => (
          <div
            key={i}
            className="bpu-page-to-pdf bg-white shadow-2xl mb-8 relative print:shadow-none print:mb-0 print:break-after-page overflow-hidden shrink-0"
            style={{
          ...
          >
            ...
            {/* ── CORPS DE PAGE ────────────────────────────────────────────── */}
            <div className="flex flex-col h-full relative" style={{ fontFamily: branding.fonts.main }}>
              {/* En-tête colonnes */}
              {page.items.length > 0 && (
                <div
              ...
              <div className="flex-col border-x border-b border-slate-200 rounded-b-lg overflow-hidden">
                {page.items.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex border-b border-slate-200 last:border-b-0 text-[10px] break-inside-avoid">
                      {/* Numéro */}
                      <div
                      ...
                      <div className="p-3 border-r border-slate-200 text-justify" style={{ width: `${COL_DESC_WIDTH}px` }}>
                        {/* Désignation — éditable inline */}
                        <div
                          contentEditable={!!setProject}
                          suppressContentEditableWarning
                          onBlur={(e) => {
                            const val      = e.currentTarget.textContent.trim();
                            const original = cleanText(item.designation);
                            if (val !== original) saveOverride(item.id, 'designation', val);
                            else if (item._overrideDesignation !== undefined) resetOverride(item.id, 'designation');
                          }}
                          className={`font-black uppercase mb-1.5 text-[11px] outline-none rounded px-0.5 -mx-0.5 transition-colors ${
                            item._overrideDesignation !== undefined
                              ? 'bg-amber-50 ring-1 ring-amber-300'
                              : setProject ? 'hover:bg-slate-50 focus:bg-white focus:ring-1 focus:ring-blue-300' : ''
                          }`}
                          style={{ color: branding.colors.text }}
                          title={setProject ? 'Cliquer pour modifier la désignation' : undefined}
                          ref={(el) => {

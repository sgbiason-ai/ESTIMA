...
describe('exportCrcArchive', () => {
  ...
  it('retourne un blob et un filename', () => {
    const { blob, filename } = exportCrcArchive(mockDoc, 'user@test.com');
    expect(blob).toBeInstanceOf(Blob);
    expect(filename).toMatch(/\.crcestima$/);
  });

  it('le filename contient le nom du chantier sanitize', () => {
    const { filename } = exportCrcArchive(mockDoc, 'user@test.com');
    expect(filename).toContain('TEST_CHANTIER');
  });

  it('le filename contient la date', () => {
    const { filename } = exportCrcArchive(mockDoc, 'user@test.com');
    const today = new Date().toISOString().split('T')[0];
    expect(filename).toContain(today);
  });

  it('le blob contient du JSON valide', async () => {
    const { blob } = exportCrcArchive(mockDoc, 'user@test.com');

    ...
    <div className="pb-2">
      <div className="flex gap-2 px-4 pt-3 pb-2 mb-1">
        <div className="flex-1 bg-white rounded-xl p-3 border border-gray-200 text-center">
          <div className="text-lg font-bold text-gray-900">{devisList.length}</div>
          <div className="text-xs text-gray-500 font-semibold">Devis</div>
        </div>
        <div className="flex-1 bg-white rounded-xl p-3 border border-gray-200 text-center">
          <div className="text-lg font-bold text-gray-900">{formatPrice(devisList.reduce((s, d) => s + d.totalHT, 0))}</div>
          <div className="text-xs text-gray-500 font-semibold">Total HT</div>
        </div>
        ...
        <button key={d.id} onClick={() => onSelect(d)}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-gray-900 leading-tight truncate">{d.nom}</div>
              {d.client && <div className="text-xs text-gray-500 font-medium mt-0.5 truncate">{d.client}</div>}

...
export default function SiteVisitListView({ visits, loading, error = '', creating = false, onSelect, onCreate, onDelete, onRefresh, isLandscape }) {
  ...
  return (
    <div className="pb-2">
      ...
      <button
        onClick={onCreate}
        disabled={creating}
        className="flex items-center justify-center gap-2 w-[calc(100%-2rem)] mx-4 mb-3 py-3 bg-gray-900 text-white rounded-xl text-[13px] font-bold active:scale-[0.97] transition disabled:opacity-50 disabled:active:scale-100"
      >
        <Icon name="plus" size={16} color="#fff" />
        {creating ? 'Création…' : 'Nouvelle visite'}
      </button>
      ...
      {!loading && error && (
        <div className="mx-4 mb-3 p-4 rounded-2xl border border-red-200 bg-red-50 text-center">
          <p className="text-[13px] font-semibold text-red-700">{error}</p>
          <button onClick={onRefresh}
            className="mt-2 px-4 py-2 rounded-xl bg-red-600 text-white text-[12px] font-bold active:scale-[0.97] transition">
            Réessayer
          </button>
        </div>
      )}
              ...
              onTouchStart={(e) => {
                ...
                if (v.isOwner) {
                  ...
                  pressTimerRef.current = setTimeout(() => {
                    ...
                    setContextMenu({ visit: v, x: touch.clientX, y: touch.clientY });
                  }, 500);
                }
              }}
      ...
      {contextMenu && (
        <>
          ...
          <div className="fixed z-50 bg-white rounded-2xl shadow-2xl border border-gray-200 py-1 w-48 overflow-hidden"
            style={{
              top: Math.max(8, Math.min(contextMenu.y + 8, window.innerHeight - 112)),

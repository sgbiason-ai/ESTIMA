...
import { formatPrice, cleanText } from '../utils/helpers';
...
const FormulaInput = ({ value, formula, formulaMode, setFormulaMode, onCommit, disabled, className, placeholder }) => {
  ...
  const getDisplayValue = () => {
    ...
    if (!isNaN(num) && inputValue !== '') return num.toFixed(0);
    return inputValue;
  };
        ...
        {(provided, snapshot) => (
          <div
            ...
            className={`flex items-center border-b border-slate-100 py-1 outline-none transition-colors
              ...
              ${formulaMode?.isActive ? 'cursor-crosshair hover:bg-amber-50 hover:ring-1 hover:ring-inset hover:ring-amber-300' : 'hover:bg-emerald-50'}
    ...
    <Draggable draggableId={draggableId} index={index} isDragDisabled={isReadOnly}>
      {(provided, snapshot) => (
        ...
        >
          ...
          >
            ...
            <div className="w-10 flex justify-center gap-1 shrink-0">
              {!isReadOnly && (
                <>
                  <button
                    onClick={(e) => { e.stopPropagation(); onAddSub(el.id); }}
          ...
          <Droppable droppableId={el.id} type="ITEM" isDropDisabled={isReadOnly}>
            {(providedDrop, snapshotDrop) => (
              <div
                ref={providedDrop.innerRef}
                {...providedDrop.droppableProps}
                className={`pl-4 border-l ml-4 min-h-[5px] transition-colors ${
              ...
              >
                ...
                {(!el.children || el.children.length === 0) && !isReadOnly && (
                  <div className="h-6 flex items-center text-[9px] text-slate-400 italic pl-2">Glisser ici...</div>
                )}
              </div>
            )}
          </Droppable>

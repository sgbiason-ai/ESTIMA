// src/components/ccap/CcapSidebar.jsx
...
const CcapSidebar = ({
  ...
  filteredCcapData, ccapDataLength,
    ...
    <div className="w-1/3 bg-white border-r border-slate-200 flex flex-col shadow-xl z-20">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-[#f3f3f3] border-b border-slate-200 shrink-0">
        <div className="flex items-center gap-2 flex-1">
          ...
          {handlePdfUpload && (
            <label className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-transparent hover:bg-[#fbe9e9] hover:border-[#f0c4c4] transition-all cursor-pointer" title="Importer un CCAP au format PDF (structure + texte reconstruits par chapitre)">
          ...
          {loadTemplate && (
            <button onClick={loadTemplate} className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-transparent hover:bg-[#dce6f0] hover:border-[#c4d5e8] transition-all" title="Charger le modèle type (remplace la structure actuelle)">
      ...
      <div className="px-3 py-2 border-b border-slate-200 bg-slate-50 flex flex-col gap-2 shrink-0">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded" title="Chapitres filtrés / total">
            {filteredCcapData.length} / {ccapDataLength}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-white">
        {renderTree(filteredCcapData)}
      </div>
    </div>
  );
};

export default CcapSidebar;

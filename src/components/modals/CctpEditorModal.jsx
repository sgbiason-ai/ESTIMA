...
import { 
  ...
  Heading1, Heading2, Eraser, Braces // Nouvelle icône pour les variables
} from 'lucide-react';
...
const CctpEditorModal = ({ isOpen, onClose, node, onSave, availableVariables }) => {
    ...
    <div className="fixed inset-0 z-modal flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="flex items-center gap-1 p-2 border-b border-slate-200 bg-slate-50 shrink-0 overflow-x-auto">
            ...
            <div className="w-px h-6 bg-slate-300 mx-2"></div>
            <ToolbarBtn onClick={() => execCmd('removeFormat')} icon={<Eraser size={16} />} label="Nettoyer" />
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-white relative editor-wrapper cursor-text" onClick={() => editorRef.current?.focus()}>
            <div 
                ref={editorRef}
                contentEditable
                className="outline-none min-h-full p-12 text-base text-slate-800 font-sans"
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-between items-center shrink-0">
          <p className="text-xs text-slate-400 italic">
            <span className="font-bold text-emerald-600">Astuce :</span> Les tableaux importés de Word sont supportés. Utilisez le bouton "Var" pour insérer des champs dynamiques.
          </p>
      ...
      <style>{`
        ...
        .editor-wrapper td, .editor-wrapper th { border: 1px solid #cbd5e1; padding: 8px 12px; min-width: 30px; vertical-align: top; position: relative; }
        .editor-wrapper th { background-color: #f1f5f9; font-weight: bold; color: #334155; }
        .editor-wrapper ul { list-style-type: disc; padding-left: 1.5em; margin: 1em 0; }
        .editor-wrapper ol { list-style-type: decimal; padding-left: 1.5em; margin: 1em 0; }
        .editor-wrapper h3 { font-size: 1.5em; font-weight: bold; margin: 1em 0 0.5em; color: #1e293b; }
        .editor-wrapper h4 { font-size: 1.25em; font-weight: bold; margin: 1em 0 0.5em; color: #334155; }
...
export default CctpEditorModal;
// src/components/crr/CrcSendMailModal.jsx
//
// Modale d'envoi du CR par email via Cloud Function SMTP.
// Le PDF est genere localement, encode base64, envoye au serveur qui le joint au mail.

import React, { useEffect, useMemo, useState } from 'react';
import {
  X, Send, Loader2, Plus, Paperclip, Mail, AlertTriangle,
  Settings as SettingsIcon, Eye,
} from 'lucide-react';
import { toast } from '../../utils/globalUI';
import { buildMailSubject, buildMailHtml, formatDateFR } from '../../utils/crrMailer';
...
const CrcSendMailModal = ({
  open, onClose,
  meeting, crrConfig, projectName, branding, sortDate, sortCat,
  companyId, crrId,
  smtpConfig,
  defaultRecipients = [],
  onOpenSmtpSettings,
  ...
  const [pdfStatus, setPdfStatus] = useState('idle'); // idle | building | ready
  ...
  useEffect(() => {
    ...
    setCc([]);
    setBcc([]);
    setSubject(buildMailSubject(meeting, projectName));
    setIntro('');
    setShowPreview(false);
    setShowCc(false);
    setShowBcc(false);
    setPdfStatus('idle');
  }, [open, meeting, projectName, defaultRecipients]);

  // Aperçu HTML : intro personnelle + body par defaut
  const htmlBody = useMemo(() => {
    if (!meeting) return '';
    const base = buildMailHtml(meeting, projectName);
    if (!intro.trim()) return base;
    const introHtml = intro.split(/\n\n+/).map((p) =>
      `<p>${htmlEsc(p).replace(/\n/g, '<br>')}</p>`
    ).join('');
    return introHtml + base;
  }, [meeting, projectName, intro]);

  if (!open) return null;

  const invalidEmails = [...to, ...cc, ...bcc].filter((e) => !isValidEmail(e));
  const canSend = to.length > 0 && invalidEmails.length === 0 && subject.trim() && smtpConfig?.isConfigured;
  ...
  const handleSend = async () => {
    ...
    try {
      // 1. Genere le PDF localement
      setPdfStatus('building');
      const { generatePdfCrr } = await import('../../utils/pdfCrrGenerator');
      const { buildExportFilename } = await import('../../utils/exportHelpers');
      const info = crrConfig?.chantierInfo || {};
      const filename = buildExportFilename(info.exportPattern, {
        number: meeting.number, projectName, date: meeting.date, ext: 'pdf',
      });
      const pdfData = await generatePdfCrr(
        meeting, crrConfig, projectName, branding,
        { returnBlob: true, filename, sortDate, sortCat }
      );
      ...
      // 3. Appelle Cloud Function
      await sendCrcEmail({
        companyId, crrId,
        meetingId: meeting.id || null,
        meetingNumber: meeting.number || null,
        to, cc, bcc,
      ...
      toast.success(`Email envoye a ${to.length} destinataire${to.length > 1 ? 's' : ''}.`);
      onClose();
    } catch (err) {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-900 text-base">Envoyer le compte rendu</h2>
              <p className="text-[11px] text-gray-500">
                {meeting?.number ? `CR n°${meeting.number}` : 'Reunion'}
                {meeting?.date ? ` — ${formatDateFR(meeting.date)}` : ''}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200">
              ...
              <div className="flex-1">
                ...
                <p className="text-[12px] text-amber-800 mb-3">
                  Pour envoyer des emails depuis le serveur (sans Outlook), vous devez d'abord configurer
                  votre serveur SMTP dans vos parametres compte.
                </p>
                <button
                  onClick={onOpenSmtpSettings}
                  className="px-3 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-[11px] font-bold uppercase tracking-wider flex items-center gap-2"
                >
                  <SettingsIcon size={12} /> Configurer SMTP
                </button>
              </div>
            </div>
          ) : (
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Ajoutez un mot personnel — sera insere en tete du message."
              className="w-full bg-gray-100 border border-gray-200/60 rounded-xl px-4 py-2.5 text-sm focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none resize-none"
            />
            <p className="text-[10px] text-gray-400 mt-1">
              Le texte standard (presentation du CR, prochaine reunion, mention 48h) est ajoute automatiquement.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-50 border border-gray-200/60 text-[12px] text-gray-700">
            ...
            Piece jointe : <strong>{pdfFilename || 'Compte rendu (genere a l\'envoi)'}</strong>
          </div>
        </div>
...
export default CrcSendMailModal;

...
import { X, Save, Plus, Hash, Copy, Folder, Check, ChevronDown, BookOpen, Search, Maximize2, Minimize2, Trash2 } from 'lucide-react';
import RichTextEditor from '../common/RichTextEditor';
import CctpSelectorModal from '../modals/CctpSelectorModal'; 

const AddBpuModal = ({ isOpen, onClose, onAdd, units, categories = [], bpuConfig, existingItems = [], duplicateItem = null, masterCctp = [] }) => {
  const initialFormState = { 
    designation: '', description: '', unit: 'u', price: 0, bpuNum: '', categoryIds: [], cctpRefs: [] // Changement : cctpRefs est un tableau
  };
  ...
  useEffect(() => {
    if (isOpen) {
      if (duplicateItem) {
        setFormData({
            designation: duplicateItem.designation + " (COPIE)", 
            description: duplicateItem.description || '',
            unit: duplicateItem.unit || 'u',
            price: duplicateItem.price || 0,
            bpuNum: "",
            // Migration : on récupère le tableau ou l'ancienne valeur unique
            cctpRefs: duplicateItem.cctpRefs || (duplicateItem.cctpRef ? [duplicateItem.cctpRef] : []),
            categoryIds: duplicateItem.categoryIds || (duplicateItem.categoryId ? [duplicateItem.categoryId] : [])
        });
        setDisplayPrice(formatPrice(duplicateItem.price || 0));
      } else {
  ...
  // Ajout d'un CCTP (Multi)
  const addCctpRef = (id) => {
      if (!id) return;
      setFormData(prev => {
          const currentRefs = prev.cctpRefs || [];
          if (currentRefs.includes(id)) return prev; // Déjà présent
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.designation || !formData.designation.trim()) { setError("La désignation est obligatoire."); return; }
    if (duplicateItem && formData.designation.includes("(COPIE)")) {
        const ok = await confirm("Vous n'avez pas modifié le titre (il contient encore 'COPIE'). Voulez-vous continuer ?");
        if (!ok) return;
    }
    if (bpuConfig?.numberingMode === 'manual') {
        const num = formData.bpuNum?.trim();
        if (num) {
            const duplicate = existingItems.find(i => i.bpuNum === num);
    ...
    onAdd({
        ...
        cctpRefs: formData.cctpRefs || [], // Envoi du tableau
        cctpRef: null, // Nettoyage ancien champ
        categoryIds: formData.categoryIds || [],
        categoryId: null
    });

    // CORRECTION : On demande à la fenêtre de se fermer après avoir validé
    onClose();
  };
    ...
    <div className={`fixed inset-0 z-modal flex items-center justify-center bg-slate-900/80 backdrop-blur-sm ${isFullScreen ? 'p-0' : 'p-4'}`}>
      <div className={`bg-white w-full shadow-2xl overflow-hidden border border-slate-200 flex flex-col relative transition-all duration-300 ${isFullScreen ? 'h-full rounded-none' : 'max-w-7xl h-[90vh] rounded-xl'}`}>
        ...
        <CctpSelectorModal 
            ...
            currentRef="" // On ne passe rien pour ne pas pré-cocher en mode "ajout unique"
            onSelect={addCctpRef} // On ajoute à la liste
        />
        ...
        <header className={`bg-slate-900 px-6 flex justify-between items-center text-white shrink-0 shadow-md transition-all duration-300 ${isFullScreen ? 'py-2 h-10' : 'py-4'}`}>
          <div className="flex items-center gap-3">
            {duplicateItem ? <Copy size={isFullScreen ? 14 : 18} className="text-blue-400" /> : <Plus size={isFullScreen ? 14 : 18} className="text-emerald-400" />}
            <h3 className={`font-black uppercase tracking-[0.2em] text-white ${isFullScreen ? 'text-[10px]' : 'text-xs'}`}>
                {duplicateItem ? "Dupliquer un Article" : "Création Nouvel Article"}
            </h3>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col h-full">
            ...
            <div className={`px-8 shrink-0 grid grid-cols-12 bg-white border-b border-slate-200 shadow-sm relative transition-all duration-300 ${isFullScreen ? 'py-3 gap-x-4 gap-y-2' : 'pb-6 pt-8 gap-6'}`}>
              ...
              {bpuConfig?.numberingMode === 'manual' && (
                  <div className="col-span-2">
                    ...
                    <input type="text" name="bpuNum" value={formData.bpuNum} onChange={handleChange} className={`${inputClass} border-blue-200 focus:border-blue-500 text-blue-800`} placeholder="Ex: 1.1" autoFocus />
                  </div>
              )}
              ...
              <div className="col-span-2">
                 ...
                 <select value={formData.unit} onChange={(e) => setFormData({...formData, unit: e.target.value})} className={`${inputClass} cursor-pointer`}>
                   {/* Article dupliqué avec une unité hors liste (ex. "M2", "m²") : on la conserve brute
                       pour rester fidèle au DQE, au lieu de retomber sur la 1re option ("u - Unité"). */}
                            ...
                            formData.cctpRefs.map(refId => {
                                ...
                                return (
                                    <span key={refId} className={`inline-flex items-center gap-1 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded px-1.5 max-w-full ${isFullScreen ? 'py-0 text-[9px]' : 'py-0.5 text-xs'}`}>
                                        ...
                                        <button type="button" onClick={(e) => removeCctpRef(refId, e)} className="hover:bg-indigo-200 rounded-full p-0.5"><X size={10}/></button>
                                    </span>
                                );
                            })
                        ) : (<span className={`text-slate-400 italic ${isFullScreen ? 'text-xs' : 'text-sm'}`}>Aucun chapitre lié...</span>)}
                    </div>
            ...
            <div className="flex-1 p-8 pt-6 flex flex-col min-h-0">
              ...
              <div className="flex-1 flex flex-col min-h-0 shadow-sm rounded-lg overflow-hidden border border-slate-300">
                <RichTextEditor value={formData.description || ""} onChange={(html) => setFormData({...formData, description: html})} placeholder="Détaillez ici les prestations techniques..." />
              </div>
            </div>
        ...
        <footer className={`flex justify-end gap-4 border-t border-slate-200 bg-white shrink-0 z-20 transition-all duration-300 ${isFullScreen ? 'p-3' : 'p-6'}`}>
          ...
          <button type="submit" onClick={handleSubmit} className="bg-emerald-600 text-white px-8 py-3 rounded-lg text-[11px] font-black uppercase tracking-[0.2em] shadow-lg hover:shadow-xl hover:bg-emerald-700 transition-all flex items-center gap-2 transform active:scale-95"><Save size={16} /> {duplicateItem ? "Créer la copie" : "Créer l'article"}</button>
        </footer>
      </div>
    </div>
  );

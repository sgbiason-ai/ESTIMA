...
const TabTechnique = ({
  ...
  const getTechCompletion = (companyName) => {
    ...
    nonAuto.forEach(crit => {
      ...
      if (hasSubs) {
        const allSubsFilled = crit.subCriteria.every(sc => Number(tech[sc.id]?.note || 0) > 0);
        if (allSubsFilled) filled++;
      } else {
        if (Number(tech[crit.id]?.note || 0) > 0) filled++;
      }
    });
    if (filled === nonAuto.length) return 'complete';
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-7xl mx-auto space-y-8 pb-32">
          ...
          <div className={`flex items-center gap-5 px-6 py-4 bg-white rounded-2xl border ${uiColor.border} border-l-[5px] shadow-sm`}>
            <div className={`w-12 h-12 rounded-[16px] ${uiColor.bg} ${uiColor.text} flex items-center justify-center font-black text-xl shadow-inner`}>
              {name.substring(0, 2).toUpperCase()}
            </div>
            <div className="flex-1">
              <h2 className="font-black text-slate-900 text-xl tracking-tight leading-none mb-1">{name}</h2>

...
import { 
  ChevronDown, MessageSquare, CheckCircle2, FileOutput, Copy, 
...
const TabNegociation = ({ 
  ...
  const generateAnomaliesText = (companyName) => {
    ...
    // chaptersData = [{ id, title, items: [{ ...node, activeQty, companyData:{[id]:{pu,...}} }] }]
    chaptersData.forEach(chapter => {
      chapter.items.forEach(item => {
        ...
        // Seuil : écart > 25% ET impact > 2% du total
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-24">
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl p-8 text-white shadow-lg relative overflow-hidden mb-6">
      ...
      {companyNames.map((name, ci) => {
        ...
        return (
          <div key={name} className={`bg-white rounded-3xl border ${isOpen ? 'border-slate-300 shadow-xl' : 'border-slate-200 shadow-sm hover:shadow-md'} overflow-hidden transition-all duration-300`}>
            <button
            ...
            >
              <div className="flex items-center gap-6">
                <div className={`w-14 h-14 rounded-2xl ${uiColor.bg} ${uiColor.text} flex items-center justify-center font-black text-xl shadow-inner`}>
                  {name.substring(0, 2).toUpperCase()}
                </div>
                <span className="font-extrabold text-slate-900 text-2xl tracking-tight">{name}</span>
                {hasContent && (
                  <span className="text-xs font-black uppercase tracking-wider bg-amber-100 text-amber-800 border border-amber-300 px-4 py-1.5 rounded-full ml-3 shadow-sm">
                    Échanges en cours
                  </span>
                )}
              </div>
              <div className={`w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 shadow-sm ${isOpen ? 'bg-slate-900 text-white rotate-180' : 'bg-white border border-slate-200 text-slate-500 group-hover:bg-slate-50'}`}>
                <ChevronDown size={22} />
              </div>
            </button>
            
            {isOpen && (
              <div className="px-8 pb-10 pt-4 bg-slate-50/50 border-t border-slate-100">
                  ...
                  <div className="bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
                    <div className="p-6 flex-1 flex flex-col">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                        ...
                        {/* BOUTON MAGIQUE DÉTECTION PRIX ATYPIQUES */}
                        <button 
